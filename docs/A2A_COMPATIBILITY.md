...
| Send | JSON-RPC `message/send` | operation `SendMessage` mapped by binding | Compatibility adapter is future coordination hardening work. |
...
| Part | custom `kind`-style payloads | protobuf oneof/member-name JSON: `text`, `raw`, `url`, or `data` | Fixtures cover each official member name; no local discriminator. |
